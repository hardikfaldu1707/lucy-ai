...
async function fetchDashboard(): Promise<DashboardSnapshot> {
  ...
  const json = (await res.json()) as { snapshot: DashboardSnapshot };
...
export function DashboardPageClient() {
  ...
  const displayName = profileData?.profile?.username ?? null;
  const welcome = displayName ? `Welcome back, ${displayName}` : "Welcome back";
  const activeCharacter = snapshot?.activeCharacter ?? null;
  const recentConversations = snapshot?.recentConversations ?? [];
